                    ...
                    if( isset( $instance['image'] ) && $instance['image']!='' )
                    {
                        foreach ($instance['image'] as $key => $value) {
                            if( isset( $instance['image'][$key] ) && $instance['image'][$key]!='' ){
                                // $image_id = $obj->raratheme_companion_get_attachment_id($instance['image'][$key]);
                                ...
                                // $path = parse_url( $instance['image'][$key], PHP_URL_PATH );
                                // $segments = explode('/', rtrim($path, '/'));
                                // $theme_slug = apply_filters('theme_slug','raratheme-companion');
                                // if( $segments[1] === 'preview' && $segments[2] === $theme_slug ){
                                //     $fimg_url = $instance['image'][$key]; /** If the img url has our demo url then simply return the url.*/
                                // }else{
